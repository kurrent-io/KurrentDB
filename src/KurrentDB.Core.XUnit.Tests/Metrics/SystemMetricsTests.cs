...
public class SystemMetricsTests : IDisposable {
	...
	[Fact]
	public void skips_drives_that_could_not_be_read() {
		// DriveStats falls back to a zero-capacity drive when it cannot stat a path. Registering it
		// would publish a permanent total=0 series and re-log the failure on every scrape.
		using var meter = new Meter($"{typeof(SystemMetricsTests)}.unreadable");
		using var listener = new TestMeterListener<long>(meter);

		DriveData GetDriveInfo(string path) => path switch {
			"/db" => new DriveData("disk-a", TotalBytes: 1000, AvailableBytes: 400),
			"/logs" => new DriveData("Unknown", TotalBytes: 0, AvailableBytes: 0),
			_ => throw new ArgumentOutOfRangeException(nameof(path), path, null),
		};
		...
		// used + total for the one readable drive, and nothing for the other.
		var measurements = listener.RetrieveMeasurements("eventstore-sys-disk-bytes");
		measurements.Select(m => (string)m.Tags.Single(t => t.Key == "disk").Value)
			.Should().Equal("disk-a", "disk-a");
	}
