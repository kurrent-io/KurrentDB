...
abstract class ApiCallback<TState, TResponse> : IEnvelope {
    protected ApiCallback(Permission permission, in TState state, string? operationName = null) {
    ...
    /// <summary>
    /// The title of the operation being performed.
    ...
    public void ReplyWith<T>(T message) where T : Message {
        try {
            ...
            try {
                Operation.TrySetException(MapToError(message, State));
            }
            catch (Exception ex) when (ex is not OperationCanceledException) {
...
) : ApiCallback<TState, TResponse>(permission, state, operationName) {
