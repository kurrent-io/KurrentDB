...
public class KontextPluginTests {
	...
	public void Defaults_Place_Files_Under_Index_Kontext_Dir() {
		...
		var checkpointConfig = new KontextCheckpointConfig { CheckpointFilesPrefix = Path.Combine(expectedDir, "checkpoint") };

		searchConfig.DataPath.ShouldStartWith(expectedIndex);
		checkpointConfig.CheckpointFilesPrefix.ShouldStartWith(expectedIndex);
		checkpointConfig.CheckpointFilesPrefix.ShouldEndWith("checkpoint");
	}
