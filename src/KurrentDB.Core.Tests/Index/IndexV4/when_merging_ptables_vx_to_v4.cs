	...
	public void midpoints_are_cached_in_ptable_footer() {
		...
		using (var filestream = File.Open(newTableFileCopy, FileMode.Open, FileAccess.Read)) {
			...
			Assert.AreEqual(filestream.Length,
				PTableHeader.Size + numIndexEntries * IndexEntry.IndexEntryV3And4Size +
				requiredMidpoints * IndexEntry.IndexEntryV3And4Size + footerSize + PTable.MD5Size);
			filestream.Seek(
				PTableHeader.Size + numIndexEntries * IndexEntry.IndexEntryV3And4Size +
				requiredMidpoints * IndexEntry.IndexEntryV3And4Size, SeekOrigin.Begin);
