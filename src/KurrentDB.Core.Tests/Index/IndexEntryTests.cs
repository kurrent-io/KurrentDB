...
public class IndexEntryTests {
	...
	[TestCaseSource(nameof(LayoutCases))]
	public void AppendTo_writes_the_versioned_binary_layout(byte ptableVersion, IndexEntry entry, byte[] expected) {
		using var stream = new MemoryStream();

		entry.AppendTo(stream, ptableVersion);

		CollectionAssert.AreEqual(expected, stream.ToArray());
	}

	[TestCase(PTableVersions.IndexV1, 16)]
	[TestCase(PTableVersions.IndexV2, 20)]
	[TestCase(PTableVersions.IndexV3, 24)]
	[TestCase(PTableVersions.IndexV4, 24)]
	public void GetSize_returns_correct_size(byte version, int expected) {
		var actual = IndexEntry.GetSize(version);
