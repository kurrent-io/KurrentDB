...
public abstract class SchemaRegistryServerTestFixture : ITestStartEventReceiver, ITestEndEventReceiver {
    ...
    public async ValueTask OnTestStart(BeforeTestContext beforeTestContext) {
        ...
        TestServer               = SchemaRegistryServerAutoWireUp.TestServer;
        HttpClient               = SchemaRegistryServerAutoWireUp.HttpClient;
        Client                   = SchemaRegistryServerAutoWireUp.Client;
        LoggerFactory            = TestServer.Services.GetRequiredService<ILoggerFactory>();
        TimeProvider             = TestServer.Services.GetRequiredService<FakeTimeProvider>();
        SchemaRegistry           = TestServer.Services.GetRequiredService<ISchemaRegistry>();
        DuckDBConnectionProvider = TestServer.Services.GetRequiredKeyedService<DuckDBConnectionProvider>("schema-registry");
    }
    ...
    protected async ValueTask<SurgeRecord> CreateRecord<T>(T message, SchemaDataFormat dataFormat = SchemaDataFormat.Json, string? streamId = null) {
        ...
        return new SurgeRecord {
            Id         = Guid.NewGuid(),
            Position   = streamId is null ? RecordPosition.ForLog(sequenceId) : RecordPosition.ForStream(streamId, StreamRevision.From((long)sequenceId), sequenceId),
