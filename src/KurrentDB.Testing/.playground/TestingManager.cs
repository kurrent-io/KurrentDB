...
// 	    var testUid = context.AssignTestUid();
//
// 	    context.ConfigureLogging(testUid);
//
// 	    var logger = AppLogging.CaptureLogs(
// 		    testUid, "TestUid",
// 		    static (uid, _) => TestContext.Current.GetTestUid(Guid.Empty).Equals(uid));
//
// 		var provider = new SerilogLoggerProvider(logger);
//
// 		var scope = provider
// 			// .CreateLogger("TestScope")
// 			.BeginScope(new Dictionary<string, object> { { "TestUid", testUid } });
//
//
...
//         );
//
//         Log.Verbose("#### Test {TestName} started", context.GetTestMethodName());
//
...
//             context.GetTestMethodName(),
//             ((context.TestEnd ?? TimeProvider.System.GetUtcNow()) - context.TestStart).Humanize(precision: 2));
//     }
//
//     public static ValueTask OnTestRetry(TestContext context, int retryAttempt) {
// 	    Log.Warning(
// 		    "#### Test {TestName} retrying (attempt {Attempt})",
// 		    context.GetTestMethodName(), retryAttempt + 1);
//
