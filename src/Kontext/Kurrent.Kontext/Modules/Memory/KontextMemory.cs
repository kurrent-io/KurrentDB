...
public sealed class KontextMemory(
	...
	public async ValueTask<Contracts.RetainResponse> RetainAsync(Contracts.RetainRequest request, CancellationToken ct = default) {
		...
		for (var i = 0; i < request.Memories.Count; i++) {
			...
			var result   = new Contracts.RetainResponse.Types.RetainResult { Outcome = decision.Outcome };

			switch (decision.Outcome) {
				case Contracts.RetainOutcome.Noop:
					result.MemoryId = decision.Existing!.MemoryId;
					break;

				case Contracts.RetainOutcome.Deferred:
		...
		static void Absorb(Contracts.Memory memory, Contracts.StoredMemory existing) {
	...
	readonly record struct RetainDecision(
		Contracts.RetainOutcome Outcome,
		Contracts.StoredMemory? Existing,
		IReadOnlyList<Contracts.RetainResponse.Types.RelatedMemory> Candidates
	);

	/// <summary>Resolves every incoming memory to an outcome, cheapest test first.</summary>
	async ValueTask<RetainDecision[]> DecideAsync(Contracts.RetainRequest request, CancellationToken ct) {
		...
		for (var i = 0; i < request.Memories.Count; i++) {
			...
			if (request.Memories[i].Supersedes.Count > 0)
				decisions[i] = new(Contracts.RetainOutcome.Merged, null, []);
			else if (request.Decided)
				decisions[i] = new(Contracts.RetainOutcome.Created, null, []);
			else
				undecided.Add(i);
		}
		...
		foreach (var i in undecided) {
			...
			decisions[i] = Covers(existing.Tags, memory.Tags)
				? new(Contracts.RetainOutcome.Noop, existing, [])
				: new(Contracts.RetainOutcome.Merged, existing, []);
		}
		...
		for (var n = 0; n < inferential.Count; n++) {
			...
			decisions[i] = distance switch {
				_ when distance < options.MergeCeiling => new(Contracts.RetainOutcome.Merged, nearest.Memory, []),
				_ when distance > options.AppendFloor  => new(Contracts.RetainOutcome.Created, null, []),
				_                                      => new(Contracts.RetainOutcome.Deferred, null, [.. hits.Select(ToRelated)]),
			};
		}
		...
		static bool Covers(IEnumerable<Contracts.Tag> stored, IEnumerable<Contracts.Tag> incoming) {
		...
		static Contracts.RetainResponse.Types.RelatedMemory ToRelated(MemoryHit hit) =>
			new() {
