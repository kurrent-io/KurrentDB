...
public class ChatRelevanceModelTests {
	...
	public async ValueTask bare_number_reply_becomes_the_score() {
		var scores = await ChatRelevanceModel.Create(new FakeChatClient("0.85")).ScoreAsync("query", ["passage"]);
	...
	public async ValueTask number_wrapped_in_prose_is_still_read() {
		var scores = await ChatRelevanceModel.Create(new FakeChatClient("The score is 0.72.")).ScoreAsync("query", ["passage"]);
	...
	public async ValueTask out_of_range_replies_clamp_to_the_unit_interval() {
		...
		var scores = await ChatRelevanceModel.Create(chat).ScoreAsync("query", ["alpha", "beta"]);
	...
	public async ValueTask neutral_fallback_honours_the_configured_score() {
		var model = ChatRelevanceModel.Create(new FakeChatClient("no idea"), options => options.NeutralScore = 0.25);
	...
	public async ValueTask first_number_in_the_reply_wins() {
		var scores = await ChatRelevanceModel.Create(new FakeChatClient("Confidence 100%. Relevance: 0.1")).ScoreAsync("query", ["passage"]);
	...
	public async ValueTask temperature_and_output_cap_reach_the_chat_client() {
		...
		var model = ChatRelevanceModel.Create(chat, options => options.MaxOutputTokens = 4);
	...
	public async ValueTask one_call_per_passage_and_scores_land_in_input_order() {
		...
		var scores = await ChatRelevanceModel.Create(chat).ScoreAsync("query", ["alpha", "beta", "gamma"]);
	...
	public async ValueTask custom_prompt_builder_is_what_the_client_sees() {
		var chat  = new FakeChatClient("0.5");
		var model = ChatRelevanceModel.Create(chat, options => options.BuildPrompt = (query, passage) => $"judge {query} :: {passage}");
	...
	public async ValueTask failing_chat_client_surfaces_its_own_failure_and_frees_the_gate() {
		...
		var model = ChatRelevanceModel.Create(chat, options => options.MaxConcurrency = 2);
