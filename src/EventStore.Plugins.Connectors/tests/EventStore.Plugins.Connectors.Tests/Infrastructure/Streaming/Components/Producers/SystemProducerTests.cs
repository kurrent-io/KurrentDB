...
namespace EventStore.Plugins.Connectors.Tests.Streaming.Producers;
...
public class SystemProducerTests(ITestOutputHelper output, StreamingFixture fixture) : StreamingTests(output, fixture) {
	...
	public async Task sends_messages(int numberOfRequests, int batchSize, string? streamId = null) {
		...
		await using var producer = SystemProducer.Builder
			.Publisher(Fixture.Publisher)
			.LoggerFactory(Fixture.LoggerFactory)
			.ProducerName($"pdr-{streamId}-{numberOfRequests:000}-{batchSize:000}")
			.EnableLogging()
			.Create();
		...
		for (var i = 0; i < actualEvents.Count; i++) {
			...
			actualMessage.Should().BeEquivalentTo(
				sentMessage, 
				options => options.WithTracing().ComparingByValue(typeof(PartitionKey)),
