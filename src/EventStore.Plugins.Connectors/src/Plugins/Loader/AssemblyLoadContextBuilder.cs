...
public class AssemblyLoadContextBuilder {
    readonly List<string> _additionalProbingPaths = new();
    readonly List<string> _resourceProbingPaths = new();
    readonly List<string> _resourceProbingSubpaths = new();
    readonly Dictionary<string, ManagedLibrary> _managedLibraries = new(StringComparer.Ordinal);
    readonly Dictionary<string, NativeLibrary> _nativeLibraries = new(StringComparer.Ordinal);
    readonly HashSet<string> _privateAssemblies = new(StringComparer.Ordinal);
    readonly HashSet<string> _defaultAssemblies = new(StringComparer.Ordinal);
    AssemblyLoadContext _defaultLoadContext = AssemblyLoadContext.GetLoadContext(Assembly.GetExecutingAssembly()) ?? AssemblyLoadContext.Default;
    string? _mainAssemblyPath;
    bool _preferDefaultLoadContext;
    bool _lazyLoadReferences;
