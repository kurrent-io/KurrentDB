...
public class ClusterTopology : IEquatable<ClusterTopology> {
    ...
    // public ClusterNode NodeById(ClusterNodeId nodeId) =>
    //     NodesById.TryGetValue(nodeId, out var node) ? node : ClusterNode.Unmapped; 
	   //
    // public ClusterNode NodeByIndex(int nodeIndex) =>
    //     NodesByIndex.TryGetValue(nodeIndex, out var node) ? node : ClusterNode.Unmapped; 
    
	// public bool IsClusterLeader(ClusterNodeId nodeId) =>
	// 	NodesById.TryGetValue(nodeId, out var node) && node.State == ClusterNodeState.Leader; 
