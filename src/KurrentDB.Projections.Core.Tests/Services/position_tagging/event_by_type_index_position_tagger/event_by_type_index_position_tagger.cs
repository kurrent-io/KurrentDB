...
public class event_by_type_index_position_tagger {
	...
	public void setup() {
		_zeroEvent = ReaderSubscriptionMessage.CommittedEventDistributed.Sample(
			...
			Guid.NewGuid(), "type1", true, Helper.UTF8NoBom.GetBytes("{}"), [], null, 10f);
		...
		_firstEvent = ReaderSubscriptionMessage.CommittedEventDistributed.Sample(
			...
			Guid.NewGuid(), "type2", true, Helper.UTF8NoBom.GetBytes("{}"), [], null, 20f);
		...
		_secondEvent = ReaderSubscriptionMessage.CommittedEventDistributed.Sample(
			...
			Guid.NewGuid(), "type1", true, Helper.UTF8NoBom.GetBytes("{}"), [], null, 30f);
		...
		_thirdEvent = ReaderSubscriptionMessage.CommittedEventDistributed.Sample(
			...
			Guid.NewGuid(), "type2", true, Helper.UTF8NoBom.GetBytes("{}"), [], null, 40f);
	}

	[Test]
	public void can_be_created() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
	...
	public void is_message_after_checkpoint_tag_before_case() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		var result = t.IsMessageAfterCheckpointTag(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(40, 35), new Dictionary<string, long> { { "type1", 2 }, { "type2", 2 } }),
			_firstEvent);
	...
	public void is_message_after_checkpoint_tag_equal_case() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		var result = t.IsMessageAfterCheckpointTag(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(30, 20), new Dictionary<string, long> { { "type1", 0 }, { "type2", 0 } }),
			_firstEvent);
		Assert.IsFalse(result);
	}
	...
	public void is_message_after_checkpoint_tag_tf_only_equal_case() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		var result = t.IsMessageAfterCheckpointTag(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(30, 20), new Dictionary<string, long> { { "type1", -1 }, { "type2", -1 } }),
			_firstEvent);
		Assert.IsFalse(result);
	}
	...
	public void is_message_after_checkpoint_tag_incompatible_streams_case() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		var result = t.IsMessageAfterCheckpointTag(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(30, 20), new Dictionary<string, long> { { "type1", -1 }, { "type3", -1 } }),
			_firstEvent);
		Assert.IsFalse(result);
	}
	...
	public void null_streams_throws_argument_null_exception() {
		Assert.Throws<ArgumentNullException>(() => { _ = new EventByTypeIndexPositionTagger(0, null); });
	}

	[Test]
	public void empty_streams_throws_argument_exception() {
		Assert.Throws<ArgumentException>(() => { _ = new EventByTypeIndexPositionTagger(0, new string[] { }); });
	}
	...
	public void streams_checkpoint_tag_is_incompatible() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		Assert.IsFalse(t.IsCompatible(
			CheckpointTag.FromStreamPositions(0, new Dictionary<string, long> { { "$et-type1", 100 }, { "$et-type2", 150 } })));
	}
	...
	public void another_events_checkpoint_tag_is_compatible() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		Assert.IsFalse(t.IsCompatible(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(100, 50), new Dictionary<string, long> { { "type1", 100 }, { "type3", 150 } })));
	}

	[Test]
	public void the_same_events_checkpoint_tag_is_compatible() {
		var t = new EventByTypeIndexPositionTagger(0, ["type1", "type2"]);
		Assert.IsTrue(t.IsCompatible(
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(100, 50), new Dictionary<string, long> { { "type1", 100 }, { "type2", 150 } })));
	}
	...
	public void can_update_by_tf_event_if_with_prior_index_position() {
		...
		var linkEvent = ReaderSubscriptionMessage.CommittedEventDistributed.Sample(
			...
			Helper.UTF8NoBom.GetBytes("0@stream2"), []);
