...
namespace KurrentDB.Projections.Core.Tests.Services.write_query_result_phase.creating;
...
abstract class specification_with_write_query_result_projection_processing_phase<TLogFormat, TStreamId> {
	...
	public void SetUp() {
		...
		_checkpointManager = new(_publisher, Guid.NewGuid());
		_emittedStreamsTracker = new();
	...
	private static PartitionStateCache GivenStateCache() {
		var stateCache = new PartitionStateCache();

		stateCache.CachePartitionState("a", new("{}", null, CheckpointTag.FromPhase(0, completed: false)));
		stateCache.CachePartitionState("b", new("{}", null, CheckpointTag.FromPhase(0, completed: false)));
		stateCache.CachePartitionState("c", new("{}", null, CheckpointTag.FromPhase(0, completed: false)));
		return stateCache;
	}
...
[TestFixture(typeof(LogFormat.V2), typeof(string))]
[TestFixture(typeof(LogFormat.V3), typeof(uint))]
class when_completed_query_processing_event<TLogFormat, TStreamId> : specification_with_write_query_result_projection_processing_phase<TLogFormat, TStreamId> {
	protected override void When() {
		_phase.Subscribe(CheckpointTag.FromPhase(1, completed: false), false);
		_phase.SetProjectionState(PhaseState.Running);
		_phase.ProcessEvent();
