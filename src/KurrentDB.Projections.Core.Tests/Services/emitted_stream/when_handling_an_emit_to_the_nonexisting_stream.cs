...
public class when_handling_an_emit_to_the_nonexisting_stream<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId> {
	...
	public void throws_if_position_is_prior_to_from_position() {
		...
		[
			new EmittedDataEvent("test_stream", Guid.NewGuid(), "type", true, "data", null, CheckpointTag.FromPosition(0, 20, 10), null)
		]));
	}
	...
	public void publishes_already_published_events() {
		_stream.EmitEvents(
		[
			new EmittedDataEvent("test_stream", Guid.NewGuid(), "type", true, "data", null, CheckpointTag.FromPosition(0, 100, 50), null)
		]);
		Assert.AreEqual(
			1,
