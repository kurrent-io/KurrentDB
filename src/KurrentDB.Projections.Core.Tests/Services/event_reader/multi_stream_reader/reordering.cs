...
namespace KurrentDB.Projections.Core.Tests.Services.event_reader.multi_stream_reader.reordering;
...
abstract class with_multi_stream_reader<TLogFormat, TStreamId> : TestFixtureWithEventReaderService<TLogFormat, TStreamId> {
	...
	protected override bool GivenHeadingReaderRunning() => true;
	...
	protected override void Given() {
		...
		_sourceDefinition = new QuerySourcesDefinition {
			...
			AllEvents = true,
			Options = new() { ReorderEvents = true, ProcessingLag = 100 }
		};
	...
	public void returns_all_events() {
		var receivedEvents = _consumer.HandledMessages.OfType<EventReaderSubscriptionMessage.CommittedEventReceived>().ToArray();
...
class when_event_commit_is_delayed<TLogFormat, TStreamId> : with_multi_stream_reader<TLogFormat, TStreamId> {
	...
	protected override IEnumerable<WhenStep> When() {
		var fromZeroPosition = CheckpointTag.FromStreamPositions(0, new Dictionary<string, long> { { "stream-a", -1 }, { "stream-b", -1 } });
		yield return new ReaderSubscriptionManagement.Subscribe(_subscriptionId, fromZeroPosition, _readerStrategy, _readerSubscriptionOptions);

		var correlationId = Guid.NewGuid();
		yield return new ClientMessage.TransactionStart(Guid.NewGuid(), correlationId, GetInputQueue(), true, "stream-a", 0, null);
		...
		yield return
			new ClientMessage.TransactionWrite(
				...
				[new Event(Guid.NewGuid(), "type1", true, "{Data: 3, Transacted=true}", "{}")], null);
		...
		yield return
			new ClientMessage.TransactionWrite(
				Guid.NewGuid(), correlationId, GetInputQueue(), true, transactionId,
				[new Event(Guid.NewGuid(), "type1", true, "{Data: 5, Transacted=true}", "{}")], null);

		correlationId = Guid.NewGuid();
		yield return new ClientMessage.TransactionCommit(Guid.NewGuid(), correlationId, GetInputQueue(), true, transactionId, null);
