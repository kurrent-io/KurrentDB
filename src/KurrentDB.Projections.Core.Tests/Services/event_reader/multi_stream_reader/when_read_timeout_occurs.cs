...
public class when_read_timeout_occurs<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId> {
	...
	public new void When() {
		...
		_eventReader.Resume();
		_streamReadACorrelationId = _consumer.HandledMessages.OfType<ClientMessage.ReadStreamEventsForward>().Last(x => x.EventStreamId == "a").CorrelationId;
		_eventReader.Handle(new ProjectionManagementMessage.Internal.ReadTimeout(_streamReadACorrelationId, "a"));
		_eventReader.Handle(
		...
		_streamReadBCorrelationId = _consumer.HandledMessages.OfType<ClientMessage.ReadStreamEventsForward>().Last(x => x.EventStreamId == "b").CorrelationId;
		_eventReader.Handle(new ProjectionManagementMessage.Internal.ReadTimeout(_streamReadBCorrelationId, "b"));
		_eventReader.Handle(
			new ClientMessage.ReadStreamEventsForwardCompleted(
	...
	public void should_attempt_another_read_for_the_timed_out_reads() {
		var streamAReads = _consumer.HandledMessages.OfType<ClientMessage.ReadStreamEventsForward>().Where(x => x.EventStreamId == "a");

		Assert.AreEqual(streamAReads.First().CorrelationId, _streamReadACorrelationId);
		Assert.AreEqual(1, streamAReads.Skip(1).Count());

		var streamBReads = _consumer.HandledMessages.OfType<ClientMessage.ReadStreamEventsForward>().Where(x => x.EventStreamId == "b");
