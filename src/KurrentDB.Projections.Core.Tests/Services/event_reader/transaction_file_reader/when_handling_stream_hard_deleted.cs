...
public class when_handling_stream_hard_deleted<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId> {
	...
	public new void When() {
		...
		_edp = new(_bus, _distributionPointCorrelationId, null, new TFPos(100, 50), _fakeTimeProvider, deliverEndOfTFPosition: false);
		_edp.Resume();
				...
				[
					...
					ResolvedEvent.ForUnresolvedEvent(
						new EventRecord(
							...
							_fakeTimeProvider.UtcNow,
							PrepareFlags.SingleWrite | PrepareFlags.TransactionBegin | PrepareFlags.TransactionEnd,
							SystemEventTypes.StreamDeleted, [1], [2]), 200)
				], null, false, 100,
	...
	public void publishes_event_reader_partition_deleted_messages() {
		var deleteds = _consumer.HandledMessages.OfType<ReaderSubscriptionMessage.EventReaderPartitionDeleted>().ToArray();
		Assert.AreEqual(1, deleteds.Length);
