...
namespace KurrentDB.Projections.Core.Tests.Services.event_reader.event_by_type_index_reader.catching_up.index_checkpoint;
...
abstract class with_some_indexed_events<TLogFormat, TStreamId> : TestFixtureWithEventReaderService<TLogFormat, TStreamId> {
	...
	protected override void Given() {
		...
		_readerStrategy = ReaderStrategy.Create(
			...
			_timeProvider,
			runAs: null);
		_readerSubscriptionOptions = new(
...
class when_index_checkpoint_is_written_while_idle<TLogFormat, TStreamId> : with_some_indexed_events<TLogFormat, TStreamId> {
	...
	protected override IEnumerable<WhenStep> When() {
		...
		yield return CreateWriteEvent("$et-type1", "$>", "2@test-stream", TFPosToMetadata(_tfPos3), isJson: false);
		...
		yield return _consumer.HandledMessages.OfType<ClientMessage.ReadStreamEventsBackward>().Single(v => v.EventStreamId == "$et");
	}
