...
namespace KurrentDB.Projections.Core.Tests.Services.event_reader.event_by_type_index_reader.catching_up.when_reordering_happens_in_event_by_type_index;

internal abstract class ReadingReorderedEventsInTheIndexTestFixture<TLogFormat, TStreamId> : TestFixtureWithEventReaderService<TLogFormat, TStreamId> {
	...
	public void returns_events_in_original_order() {
		...
		Assert.That(
			new long[] { 0, 1, 2 }.SequenceEqual(receivedEvents.Select(e => e.Data.EventSequenceNumber)),
...
class when_starting_with_empty_index<TLogFormat, TStreamId> : ReadingReorderedEventsInTheIndexTestFixture<TLogFormat, TStreamId> {
	...
	protected override IEnumerable<WhenStep> When() {
		var fromZeroPosition =
			CheckpointTag.FromEventTypeIndexPositions(0, new TFPos(0, -1), new Dictionary<string, long> { { "type1", -1 }, { "type2", -1 } });
