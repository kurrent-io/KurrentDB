...
public class when_handling_eof_and_idle_eof<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId> {
	...
	public new void When() {
		...
		_edp = new(_bus, _distributionPointCorrelationId, null, "stream", 10, _fakeTimeProvider, false, produceStreamDeletes: false);
		_edp.Resume();
		...
		correlationId = ((ClientMessage.ReadStreamEventsForward)_consumer.HandledMessages.OfType<AwakeServiceMessage.SubscribeAwake>().Last().ReplyWithMessage)
			.CorrelationId;
		_edp.Handle(
			new ClientMessage.ReadStreamEventsForwardCompleted(
				correlationId, "stream", 100, 100, ReadStreamResult.Success,
