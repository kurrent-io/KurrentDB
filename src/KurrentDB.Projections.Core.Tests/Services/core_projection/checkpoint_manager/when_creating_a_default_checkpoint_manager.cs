...
public class when_creating_a_default_checkpoint_manager<TLogFormat, TStreamId> : TestFixtureWithCoreProjectionCheckpointManager<TLogFormat, TStreamId> {
	...
	public void it_can_be_created() {
		_manager = new DefaultCheckpointManager(
			_bus, _projectionCorrelationId, new ProjectionVersion(1, 0, 0), null, _ioDispatcher, _config, new StreamPositionTagger(0, "stream"), _namingBuilder, _checkpointsEnabled, _coreProjectionCheckpointWriter, Opts.MaxProjectionStateSizeDefault);
	}
	...
	public void null_publisher_throws_argument_null_exception() {
		Assert.Throws<ArgumentNullException>(() => {
			_manager = new DefaultCheckpointManager(
				null, _projectionCorrelationId, new ProjectionVersion(1, 0, 0), null, _ioDispatcher, _config, new StreamPositionTagger(0, "stream"), _namingBuilder, _checkpointsEnabled, _coreProjectionCheckpointWriter, Opts.MaxProjectionStateSizeDefault);
		});
	}
	...
	public void null_io_dispatcher_throws_argument_null_exception() {
		Assert.Throws<ArgumentNullException>(() => {
			_manager = new DefaultCheckpointManager(
				...
				new StreamPositionTagger(0, "stream"), _namingBuilder, _checkpointsEnabled, _coreProjectionCheckpointWriter, Opts.MaxProjectionStateSizeDefault);
		});
	}
	...
	public void null_position_tagger_throws_argument_null_exception() {
		Assert.Throws<ArgumentNullException>(() => {
			_manager = new DefaultCheckpointManager(
				_bus, _projectionCorrelationId, new ProjectionVersion(1, 0, 0), null, _ioDispatcher, _config, null, _namingBuilder, _checkpointsEnabled, _coreProjectionCheckpointWriter, Opts.MaxProjectionStateSizeDefault);
		});
	}
