...
public class when_emitting_events_with_maximum_allowed_writes_in_flight_set<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId> {
	...
	private readonly int _maximumNumberOfAllowedWritesInFlight;
	...
	public void setup() {
		...
		[
			new(new EmittedDataEvent("stream1", Guid.NewGuid(), "type1", true, "data1", null, CheckpointTag.FromPosition(0, 120, 110), null)),
			new(new EmittedDataEvent("stream2", Guid.NewGuid(), "type2", true, "data2", null, CheckpointTag.FromPosition(0, 120, 110), null)),
			new(new EmittedDataEvent("stream3", Guid.NewGuid(), "type3", true, "data3", null, CheckpointTag.FromPosition(0, 120, 110), null))
		]);
	}

	[Test]
	public void should_have_the_same_number_writes_in_flight_as_configured() {
		var writeEvents = _consumer.HandledMessages.OfType<ClientMessage.WriteEvents>().ExceptOfEventType(SystemEventTypes.StreamMetadata);
...
public class when_emitting_events_with_maximum_allowed_writes_in_flight_set_to_unlimited<TLogFormat, TStreamId>
