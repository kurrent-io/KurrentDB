...
public class
	...
	public void write_events_in_correct_order() {
		Assert.AreEqual(
			...
			Helper.UTF8NoBom.GetString(_writeEventHandler.HandledMessages.Single(v => v.EventStreamId == "/emit2").Events[0].Data));
		Assert.AreEqual(
			FakeProjectionStateHandler._emit2Data,
			Helper.UTF8NoBom.GetString(_writeEventHandler.HandledMessages.Single(v => v.EventStreamId == "/emit2").Events[1].Data));
	}
}
