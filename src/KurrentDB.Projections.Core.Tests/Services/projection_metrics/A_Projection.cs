	...
	public class Base<TLogFormat, TStreamId> : a_new_posted_projection.Base<TLogFormat, TStreamId> {
		...
		protected IEnumerable<Measurement<long>> ObservedStatus() {
			...
			if (_noStatsYet) {
				...
				ProjectionMetricTracker.OnNewStats(s.Single().Projections);
				_noStatsYet = false;
			}

			return ProjectionMetricTracker.ObserveStatus();
		}
		...
		protected int ObservedRunning() {
			...
			return (int)ProjectionMetricTracker.ObserveRunning().Single().Value;
		}
