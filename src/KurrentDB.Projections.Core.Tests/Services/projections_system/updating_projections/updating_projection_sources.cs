...
namespace KurrentDB.Projections.Core.Tests.Services.projections_system.updating_projections.updating_projection_sources;

public abstract class with_updated_projection<TLogFormat, TStreamId> : with_projection_config<TLogFormat, TStreamId> {
	private Statistics _allStatistics;
	protected ProjectionState _state;
	private ProjectionQuery _query;
	...
	protected override IEnumerable<WhenStep> When() {
		...
		yield return new Command.Post(
			...
			RunAs.System, "js", GivenOriginalSource(), true,
		...
		yield return new Command.Disable(Envelope, _projectionName, RunAs.System);
		yield return new Command.UpdateQuery(Envelope, _projectionName, RunAs.System, GivenUpdatedSource(), _emitEnabled);
		yield return CreateWriteEvent("stream2", "type3", "{\"Data\": 6}");
		yield return CreateWriteEvent("stream3", "type4", "{\"Data\": 7}");
		yield return new Command.Enable(Envelope, _projectionName, RunAs.System);
		...
		yield return new Command.GetStatistics(Envelope, ProjectionMode.AllNonTransient, _projectionName);
		yield return new Command.GetState(Envelope, _projectionName, "");
		yield return new Command.GetQuery(Envelope, _projectionName, RunAs.Anonymous);

		_allStatistics = HandledMessages.OfType<Statistics>().LastOrDefault();
		_statistics = _allStatistics?.Projections.SingleOrDefault();
		_state = HandledMessages.OfType<ProjectionState>().LastOrDefault();
		_stateData = _state != null ? EatException(() => _state.State.ParseJson<JObject>()) : null;
		_query = HandledMessages.OfType<ProjectionQuery>().LastOrDefault();
	}
...
public class when_adding_an_event_type<TLogFormat, TStreamId> : with_updated_projection<TLogFormat, TStreamId> {
	...
	public void projection_position_is_correct() {
		...
		Assert.AreEqual(
			CheckpointTag.FromEventTypeIndexPositions(0, pos, new Dictionary<string, long> { { "type1", 1 }, { "type3", 1 } }),
			_state.Position);
	}
}
