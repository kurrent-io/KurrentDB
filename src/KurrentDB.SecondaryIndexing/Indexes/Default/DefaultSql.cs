...
internal static class DefaultSql {
	...
	public record struct LastPositionResult(long PreparePosition, long? CommitPosition, long RowId, long Created);

	/// <summary>
	/// Get the last indexed log position
	/// </summary>
	public struct GetLastLogPositionQuery : IQuery<LastPositionResult> {
		public static ReadOnlySpan<byte> CommandText => "select log_position, commit_position, rowid, created from idx_all order by rowid desc limit 1"u8;

		public static bool UseStreamingMode => false;

		public static LastPositionResult Parse(ref DataChunk.Row row) => new(row.ReadInt64(), row.TryReadInt64(), row.ReadInt64(), row.ReadInt64());
	}
}
