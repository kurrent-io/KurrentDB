...
internal class CategoryIndexProcessor : Disposable, ISecondaryIndexProcessor {
	...
	private readonly Dictionary<long, (long Sequence, long Committed)> _categorySizes = new();
	...
	public void Index(ResolvedEvent resolvedEvent) {
		...
		if (_categories.TryGetValue(categoryName, out var categoryId)) {
			var current = _categorySizes[categoryId];
			var next = current.Sequence + 1;
			_categorySizes[categoryId] = (next, current.Committed);
	...
	public long GetLastEventNumber(long categoryId) =>
		_categorySizes.TryGetValue(categoryId, out var size) ? size.Committed : ExpectedVersion.NoStream;
	...
	public void Commit() {
		...
		var toCommit = _categorySizes.Where(cs => cs.Value.Sequence != cs.Value.Committed).ToList();

		foreach (var pair in toCommit) {
			_categorySizes[pair.Key] = (pair.Value.Sequence, pair.Value.Sequence);
		}
