...
static class MemoryRecordFilters {
    ...
    /// <summary>
    /// The retract cascade's fan-out: non-retracted memories whose evidence cites the given id (a
    /// derived memory citing a retracted one loses its evidence).
    /// </summary>
    public static Expression<Func<MemoryRecord, bool>> DerivedFrom(string citedMemoryId) =>
        r => r.IsRetracted == false && r.CitedMemoryIds.Contains(citedMemoryId);
