...
public class CustomIndexesServiceTests {
	...
	public async ValueTask can_create_started_by_default(CancellationToken ct) {
		...
		await can_get(customIndexName, IndexStatus.Started, ct);
	}
	...
	public async ValueTask can_create(CancellationToken ct) {
		await Client.CreateIndexAsync(
			...
			cancellationToken: ct);
		await can_get(CustomIndexName, IndexStatus.Stopped, ct);
	...
	public async ValueTask cannot_create_different(CancellationToken ct) {
		...
		await Assert.That(ex!.Status.Detail).IsEqualTo($"Index '{CustomIndexName}' already exists");
	...
	public async ValueTask can_list(CancellationToken ct) {
		...
		await Assert.That(customIndexState!.Status).IsEqualTo(IndexStatus.Stopped);
	}
	...
	public async ValueTask can_delete(CancellationToken ct) {
		...
		// no longer listed
		var response = await Client.ListIndexesAsync(new(), cancellationToken: ct);
		await Assert.That(response!.Indexes).DoesNotContainKey(CustomIndexName);
	}
	...
	async ValueTask can_get(string customIndexName, IndexStatus expectedStatus, CancellationToken ct) {
