...
public interface IIndexWriter<TStreamId> {
	...
	void PreCommit(ReadOnlySpan<IPrepareLogRecord<TStreamId>> commitedPrepares, ReadOnlySpan<int> streamIndexes);
...
public class IndexWriter<TStreamId> : IndexWriter, IIndexWriter<TStreamId> {
	...
	public void PreCommit(ReadOnlySpan<IPrepareLogRecord<TStreamId>> committedPrepares, ReadOnlySpan<int> streamIndexes) {
		ArgumentOutOfRangeException.ThrowIfNotEqual(streamIndexes.Length, committedPrepares.Length, nameof(streamIndexes));
		...
		Span<bool> streamProcessed = streamIndexes.Length < 1024 / sizeof(bool) ?
			stackalloc bool[streamIndexes.Length] :
			new bool[streamIndexes.Length];
