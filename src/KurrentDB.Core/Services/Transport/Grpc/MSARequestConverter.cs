	...
	public ClientMessage.WriteEvents ConvertRequests(
		...
		try {
			var writeEvents = new ClientMessage.WriteEvents(
				...
				envelope: envelope,
				requireLeader: true,
				eventStreamIds: eventStreamIds.ToImmutable().AsMemory(),
				expectedVersions: expectedVersions.ToImmutable().AsMemory(),
				events: events.ToImmutable().AsMemory(),
				eventStreamIndexes: eventStreamIndexes.ToImmutable().AsMemory(),
				user: user,
				cancellationToken: token);
