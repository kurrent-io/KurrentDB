			...
			case OperationResult.Success: {
				...
				for (var i = 0; i < completed.LastEventNumbers.Length; i++) {
					multiResponse.Success.Output.Add(new AppendStreamSuccess {
						...
						// todo: response Position needs ironing out.
						// it is populated with a position for the transaction - which is the same for every stream
						//    - we could change the meaning so its different per stream
						//    - we could move the Position a level higher so it isn't repeated
						//    - we could leave it as is, if its a convenient form for the client to consume for its purposes.
