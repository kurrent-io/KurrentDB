...
public class CheckpointTag : IComparable<CheckpointTag> {
	...
	private CheckpointTag(int phase, IDictionary<string, long> streams) {
		...
		Position = new(long.MinValue, long.MinValue);
		Mode_ = CalculateMode();
	}
	...
	private Mode CalculateMode() {
		...
		if (Position != new TFPos(long.MinValue, long.MinValue))
			return Mode.EventTypeIndex;
		return Streams.Count == 1 ? Mode.Stream : Mode.MultiStream;
	}
	...
	public long? PreparePosition => Mode_ switch {
	...
	public static CheckpointTag FromByStreamPosition(int phase,
		string catalogStream,
		long catalogPosition,
		string dataStream,
		long dataPosition,
	...
	public bool Completed => Position.CommitPosition == long.MaxValue;
		...
		return new() {
			Tag =
				byStreamMode
					...
					: new CheckpointTag(
						projectionPhase,
						new TFPos(commitPosition ?? long.MinValue, preparePosition ?? long.MinValue), streams),
