...
public class RequestResponseQueueForwarder(IPublisher inputQueue, IPublisher externalRequestQueue) : IHandle<ClientMessage.ReadEvent>,
	...
	public void Handle(SystemMessage.SubSystemInitialized msg) {
		externalRequestQueue.Publish(new SystemMessage.SubSystemInitialized(msg.SubSystemName));
	}
	...
	void IHandle<ProjectionCoreServiceMessage.SubComponentStopped>.Handle(
		...
		externalRequestQueue.Publish(new ProjectionCoreServiceMessage.SubComponentStopped(message.SubComponent, message.QueueId));
	}
}
