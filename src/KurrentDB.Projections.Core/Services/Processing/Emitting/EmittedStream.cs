...
public partial class EmittedStream
	: IDisposable, IHandle<CoreProjectionProcessingMessage.EmittedStreamWriteCompleted> {
	...
	private readonly Stack<(CheckpointTag, string, long)> _alreadyCommittedEvents = new();
	...
	private bool CollectAlreadyCommittedEvents(
		...
		foreach (var e in message.Events) {
			...
			_alreadyCommittedEvents.Push((checkpointTagVersion.Tag, eventType, e.Event.EventNumber));
		}
	...
	private static IEnumerable<KeyValuePair<string, JToken>> MetadataWithCausedByAndCorrelationId(EmittedEvent emittedEvent) {
		...
		if (emittedEvent.CausedBy != Guid.Empty)
			yield return
				new("$causedBy", JValue.CreateString(emittedEvent.CausedBy.ToString("D")));
	...
	private void PublishWriteEvents(int retryCount) {
		...
		if (attempt >= MinAttemptWarnThreshold && _logger != null) {
			_logger.Warning("Attempt: {attempt} to write events to stream {stream}. Backing off for {time} second(s).",
				attempt, _streamId, delayInSeconds);
		}
	...
	private IValidatedEmittedEvent ValidateEmittedEventInRecoveryMode(EmittedEvent eventToWrite) {
		...
		switch (failed) {
			case true when eventToWrite.EventType.Equals(LinkEventType): {
				...
				return !long.TryParse(parts[0], out long eventNumber)
					? throw new Exception(
					...
					: new EmittedEventResolutionNeeded(streamId, eventNumber, topAlreadyCommitted);
			}
			case true: {
	...
	private void OnEmittedLinkEventResolved(bool anyFound,
		...
		(CheckpointTag, string, long) topAlreadyCommitted,
	...
	private InvalidEmittedEventSequenceException
		CreateSequenceException((CheckpointTag, string, long) committed, EmittedEvent eventToWrite) {
		return new(
