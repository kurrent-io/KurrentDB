...
public class EmittedStreamsDeleter(
	...
	private void ReadCompleted(ClientMessage.ReadStreamEventsForwardCompleted onReadCompleted, Action onEmittedStreamsDeleted) {
		if (onReadCompleted.Result is not (ReadStreamResult.Success or ReadStreamResult.NoStream)) return;
					...
					SystemAccounts.System, x => {
						switch (x.Result) {
							...
							case OperationResult.WrongExpectedVersion:
								...
								Log.Information("PROJECTIONS: Projection Stream '{stream}' was not deleted since it does not exist",
			...
			SystemAccounts.System, x => {
				Log.Debug(
					x.Result == OperationResult.Success
						? "PROJECTIONS: Emitted Stream Deletion Checkpoint written at {eventNumber}"
