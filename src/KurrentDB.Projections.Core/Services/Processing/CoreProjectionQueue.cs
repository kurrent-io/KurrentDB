...
public class CoreProjectionQueue(IPublisher publisher, int pendingEventsThreshold, bool orderedPartitionProcessing) {
	...
	public void ProcessEvent() {
		if (_queuePendingEvents.Count == 0 && _subscriptionPaused && !_unsubscribed) {
			ResumeSubscription();
		}
	}
	...
	public void EnqueueTask(WorkItem workItem, CheckpointTag workItemCheckpointTag, bool allowCurrentPosition) {
	...
	public void EnqueueOutOfOrderTask(WorkItem workItem) {
		if (_lastEnqueuedEventTag == null)
			throw new InvalidOperationException("Cannot enqueue an out-of-order task.  The projection position is currently unknown.");
		workItem.SetProjectionQueue(this);
	...
	private void ValidateQueueingOrder(CheckpointTag eventTag, bool allowCurrentPosition = false) {
		if (eventTag < _lastEnqueuedEventTag ||
			...
			throw new InvalidOperationException($"Invalid order.  Last known tag is: '{_lastEnqueuedEventTag}'.  Current tag is: '{eventTag}'");
