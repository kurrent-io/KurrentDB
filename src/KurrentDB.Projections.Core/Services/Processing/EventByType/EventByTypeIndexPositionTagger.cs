...
public class EventByTypeIndexPositionTagger : PositionTagger {
	...
	public EventByTypeIndexPositionTagger(int phase, string[] eventTypes, bool includeStreamDeletedNotification = false) : base(phase) {
		ArgumentNullException.ThrowIfNull(eventTypes);
		if (eventTypes.Length == 0)
			throw new ArgumentException(null, nameof(eventTypes));
	...
	public override bool IsMessageAfterCheckpointTag(CheckpointTag previous,
	...
	public override CheckpointTag MakeZeroCheckpointTag()
		=> CheckpointTag.FromEventTypeIndexPositions(Phase, new(0, -1), _eventTypes.ToDictionary(v => v, _ => ExpectedVersion.NoStream));

	public override bool IsCompatible(CheckpointTag checkpointTag) {
		//TODO: should Stream be supported here as well if in the set?
		return checkpointTag.Mode_ == CheckpointTag.Mode.EventTypeIndex && checkpointTag.Streams.All(v => _eventTypes.Contains(v.Key));
	}
	...
	public override CheckpointTag AdjustTag(CheckpointTag tag) {
		...
		return tag.Mode_ switch {
			CheckpointTag.Mode.MultiStream => throw new NotSupportedException("Conversion from MultiStream to EventTypeIndex position tag is not supported"),
			CheckpointTag.Mode.Stream => throw new NotSupportedException("Conversion from Stream to EventTypeIndex position tag is not supported"),
			CheckpointTag.Mode.PreparePosition => throw new NotSupportedException("Conversion from PreparePosition to EventTypeIndex position tag is not supported"),
			CheckpointTag.Mode.Position => CheckpointTag.FromEventTypeIndexPositions(tag.Phase, tag.Position, _eventTypes.ToDictionary(v => v, _ => (long)-1)),
			_ => throw new NotSupportedException($"The given checkpoint is invalid. Possible causes might include having written an event to the projection's managed stream. The bad checkpoint: {tag}")
		};
	}
}
