...
public partial class EventByTypeIndexEventReader {
	private class IndexBased
		...
		private readonly HashSet<Guid> _validRequests = [];
		private bool _indexCheckpointStreamRequested;
		private long _lastKnownIndexCheckpointEventNumber = -1;
		private TFPos? _lastKnownIndexCheckpointPosition;

		private readonly Dictionary<string, Queue<PendingEvent>> _buffers = new();
		private readonly Dictionary<string, bool> _eofs;
		private bool _disposed;
		private bool _indexStreamEof;
		...
		public IndexBased(HashSet<string> eventTypes, EventByTypeIndexEventReader reader, ClaimsPrincipal readAs)
			...
			_eofs = _streamToEventType.Keys.ToDictionary(v => v, _ => false);
			...
			_publisher = _reader.Publisher;
			_pendingRequests = new() { { "$et", Guid.Empty } };
			foreach (var stream in _streamToEventType.Keys) {
				_pendingRequests.Add(stream, Guid.Empty);
			}
		}

		private static TFPos GetTargetEventPosition(PendingEvent head) => head.TfPosition;
		...
		private void EnqueueEvents(ClientMessage.ReadStreamEventsForwardCompleted message) {
			for (int index = 0; index < message.Events.Count; index++) {
				...
				var tfPosition = positionEvent.Metadata.ParseCheckpointTagJson().Position;
		...
		private Queue<PendingEvent> GetStreamQueue(EventRecord positionEvent) {
			if (!_buffers.TryGetValue(positionEvent.EventStreamId, out var queue)) {
				queue = new();
		...
		private void ReadIndexCheckpointStreamCompleted(
			...
			IReadOnlyList<KurrentDB.Core.Data.ResolvedEvent> events) {
			if (_disposed)
				return;
			...
			switch (result) {
				...
				case ReadStreamResult.Success:
					if (events is []) {
						_indexStreamEof = true;
						_lastKnownIndexCheckpointPosition ??= default;
					} else {
					...
					break;
				default:
					throw new NotSupportedException($"ReadEvents result code was not recognized. Code: {result}");
			}
		...
		private void ProcessBuffers() {
			...
			while (true) {
				...
				foreach (var streamId in _streamToEventType.Keys) {
					_buffers.TryGetValue(streamId, out var buffer);
		...
		private void RequestEvents(string stream, bool delay) {
			...
			var readEventsForward = new ClientMessage.ReadStreamEventsForward(
				...
				false, null, _readAs, replyOnExpired: false);
		...
		private void DeliverEventRetrievedByIndex(KurrentDB.Core.Data.ResolvedEvent pair, float progress, TFPos position) {
		...
		public override bool AreEventsRequested() => _eventsRequested.Count != 0 || _indexCheckpointStreamRequested;
		...
		private bool ShouldSwitch() {
			...
			if (_reader.Paused || _reader.PauseRequested)
				return false;
			var shouldSwitch =
				_lastKnownIndexCheckpointPosition != null
				&& _streamToEventType.Keys.All(v
					=> _eofs[v] || _buffers.TryGetValue(v, out var q) && q.Count > 0 &&
