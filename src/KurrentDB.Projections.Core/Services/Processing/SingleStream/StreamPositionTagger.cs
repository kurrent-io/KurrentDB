...
public class StreamPositionTagger : PositionTagger {
	...
	public override CheckpointTag MakeCheckpointTag(
		...
		return CheckpointTag.FromStreamPosition(previous.Phase, partitionDeleted.PositionStreamId, partitionDeleted.PositionEventNumber.Value);
	}
	...
	public override CheckpointTag AdjustTag(CheckpointTag tag) {
		...
		if (tag.Mode_ == CheckpointTag.Mode.Stream) {
			return CheckpointTag.FromStreamPosition(
				tag.Phase, _stream, tag.Streams.TryGetValue(_stream, out var p) ? p : -1);
		}
		...
		return tag.Mode_ switch {
			...
			CheckpointTag.Mode.MultiStream => CheckpointTag.FromStreamPosition(tag.Phase, _stream,
				tag.Streams.TryGetValue(_stream, out var p) ? p : -1),
			CheckpointTag.Mode.Position => throw new NotSupportedException(
				"Conversion from Position to Stream position tag is not supported"),
