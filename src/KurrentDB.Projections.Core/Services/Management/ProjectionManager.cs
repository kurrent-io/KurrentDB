...
public class ProjectionManager
	...
	private readonly RequestResponseDispatcher<ClientMessage.ReadStreamEventsForward, ClientMessage.ReadStreamEventsForwardCompleted>
		_readForwardDispatcher;

	private readonly ReadDispatcher _readDispatcher;

	private const int ReadEventsBatchSize = 100;
	...
	public void Handle(ProjectionSubsystemMessage.StartComponents message) {
		if (_started) {
			_logger.Debug("PROJECTIONS: Projection manager already started. Correlation: {correlation}", message.InstanceCorrelationId);
			return;
		}
	...
	public void Handle(ProjectionSubsystemMessage.StopComponents message) {
		if (!_started) {
			_logger.Debug("PROJECTIONS: Projection manager already stopped. Correlation: {correlation}", message.InstanceCorrelationId);
			return;
		}
	...
	private void ScheduleExpire() {
		if (!_projectionsStarted)
			return;
		_publisher.Publish(
			TimerMessage.Schedule.Create(TimeSpan.FromSeconds(60), _publishEnvelope,
		...
		if (message.Mode == ProjectionMode.Transient) {
			...
			if (_isWritePending) {
				...
				var pendingProjections = new Dictionary<string, PendingProjection> {
					{ message.Name, new(expectedVersion + 1, message) }
				};
	...
	public void Handle(ProjectionManagementMessage.Command.PostBatch message) {
		...
		if (message.Projections.Any(p => p.Mode == ProjectionMode.Transient)) {
			message.Envelope.ReplyWith(
				new ProjectionManagementMessage.OperationFailed("Transient projections in batches are not supported."));
			return;
		}
		...
		if (_isWritePending) {
			...
			foreach (var projection in message.Projections) {
				pendingProjections.Add(projection.Name, new(projectionId, projection));
				projectionId++;
			}
	...
	private bool ValidateProjections(
		...
		foreach (var projection in projections) {
			if (!ProjectionManagementMessage.RunAs.ValidateRunAs(
				...
				message.Envelope.ReplyWith(new ProjectionManagementMessage.OperationFailed("Invalid RunAs"));
				return false;
			}

			if (string.IsNullOrWhiteSpace(projection.Name)) {
				message.Envelope.ReplyWith(new ProjectionManagementMessage.OperationFailed("Projection name is required"));
				return false;
			}
		...
		if (duplicateNames.Count != 0) {
			...
			_logger.Debug("PROJECTIONS: Conflict. {DuplicatesMsg}", duplicatesMsg);
			message.Envelope.ReplyWith(new ProjectionManagementMessage.Conflict(duplicatesMsg));
			return false;
		}
	...
	public void Handle(ProjectionManagementMessage.Command.Delete message) {
		...
		if (IsSystemProjection(message.Name)) {
			message.Envelope.ReplyWith(
				new ProjectionManagementMessage.OperationFailed("We currently don't allow for the deletion of System Projections."));
			return;
		}

		if (!ProjectionManagementMessage.RunAs.ValidateRunAs(projection.Mode, ReadWrite.Write, message))
			return;
		try {
	...
	private static bool IsSystemProjection(string name)
		=> name is ProjectionNamesBuilder.StandardProjections.EventByCategoryStandardProjection
	...
	public void Handle(ProjectionManagementMessage.Command.GetQuery message) {
		...
		if (projection == null)
			...
			if (!ProjectionManagementMessage.RunAs.ValidateRunAs(projection.Mode, ReadWrite.Read, message))
				return;
			projection.Handle(message);
		}
	}
	...
	public void Handle(ProjectionManagementMessage.Command.UpdateQuery message) {
		if (!_projectionsStarted)
			return;
		_logger.Information("Updating '{projection}' projection source to '{source}`", message.Name, message.Query);
		var projection = GetProjection(message.Name);
	...
	public void Handle(ProjectionManagementMessage.Command.GetStatistics message) {
		...
		if (!string.IsNullOrEmpty(message.Name)) {
			...
			if (projection == null)
				...
				message.Envelope.ReplyWith(new ProjectionManagementMessage.Statistics([projection.GetStatistics()]));
		} else {
	...
	private void DeleteProjection(
		...
		_writeDispatcher.Publish(writeDelete, m => WriteProjectionDeletedCompleted(m, writeDelete, message, completed, retryCount));
	}
	...
	private ManagedProjection GetProjection(string name) => _projections.TryGetValue(name, out var result) ? result : null;
	...
	private bool IsProjectionEnabledToRunByMode(string projectionName)
		=> _runProjections >= ProjectionType.All || _runProjections == ProjectionType.System && projectionName.StartsWith('$');
	...
	private static ProjectionManagementMessage.Command.PostBatch.ProjectionPost CreateSystemProjectionPost(string name, Type handlerType, string config) {
		return new(
	...
	private void PostNewProjections(IDictionary<string, PendingProjection> newProjections, long expectedVersion, IEnvelope replyEnvelope) {
		...
		foreach (var projection in newProjections.Values) {
			if (projection.Mode >= ProjectionMode.OneTime) {
				...
				events.Add(new(eventId, ProjectionEventTypes.ProjectionCreated, false, projection.Name.AsUTF8Bytes()));
			} else {
		...
		_isWritePending = true;
		_writeDispatcher.Publish(writeEvents, m => WriteNewProjectionsCompleted(m, writeEvents, newProjections, replyEnvelope));
	}
	...
	private void StartNewlyRegisteredProjections(IDictionary<string, PendingProjection> newProjections,
		Action completedAction,
		IEnvelope replyEnvelope) {
		if (!newProjections.Any()) {
			replyEnvelope.ReplyWith(new ProjectionManagementMessage.OperationFailed("Projections were invalid"));
			return;
		}
	...
	private void ReadProjectionPossibleStream(string projectionName, Action<ClientMessage.ReadStreamEventsBackwardCompleted> onComplete) {
		var corrId = Guid.NewGuid();
	...
	private void WriteProjectionDeletedCompleted(ClientMessage.WriteEventsCompleted writeCompleted,
		...
		switch (writeCompleted.Result) {
			case OperationResult.CommitTimeout or OperationResult.ForwardTimeout or OperationResult.PrepareTimeout when retryCount > 0:
			...
			case OperationResult.WrongExpectedVersion:
				_logger.Error("PROJECTIONS: Got wrong expected version writing projection deletion for {projection}.", message.Name);
				return;
			default:
				_logger.Error(
	...
	private class NewProjectionInitializer {
		...
		public void CreateAndInitializeNewProjection(
			...
			Guid workerId,
			long? version = -1) {
			var projection = projectionManager.CreateManagedProjectionInstance(_name, _projectionId, projectionCorrelationId, workerId);
