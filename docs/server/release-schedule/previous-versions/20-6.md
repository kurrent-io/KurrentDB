...
Currently the easiest way to run EventStoreDB is to run in Development mode. You can do this by running Event Store with the `--dev` flag.
...
* At least one IP address or DNS name needs to be specified in the SAN. The specified IP addresses or DNS names must match the IP/host of the Event Store node.
* The certificate's subject needs to be `CN=eventstoredb-node`.
* You will also need to configure Event Store with the path to the CA used to create these certificates.
...
You will need to either trust the certificate authority you are running Event Store with, or disable remote certificate validation on your client.
...
HTTP prefixes were dropped with the move from HTTPListener to Kestrel. Please be aware that Event Store will fail if you try to specify HTTP Prefixes in the configuration.
...
With Event Store 20.6.0 we wanted to move to a secure by default approach and therefore the Tcp Client has been changed to have Tls Enabled by default.
...
With the release of Event Store 20.6.0, the logging system moved over to Serilog. This does mean that the previous logging configuration is no longer valid for users who are upgrading.
...
Although macOS was never a supported server platform for Event Store, we have historically shipped binaries for those developing on that platform. In this preview release we have completed the move to gRPC for both client access and gossip which enables TLS by default.
...
Event Store 20.6.0 only runs on Core CLR 3.1 which means that support for both the .NET Framework and Mono have been dropped.
...
To improve this situation, we have added a new default client protocol in Event Store 20.6.0, using gRPC - a widely adopted standard which is interoperable across lots of platforms with minimal effort.

With Event Store 20.6.0, we are shipping a .NET SDK for the new gRPC protocol.

#### Gossip over gRPC

With the preview releases of Event Store 20.6.0, we’ve had great feedback from community members building out gRPC Clients and with that requested that we expose the cluster gossip protocol over gRPC for cluster aware clients to use.

The HTTP based gossip has not been removed and any existing operational tools and clients such the Event Store .NET TCP Client will continue to work, however due to the role name change, the consumers of the protocol will have to be updated to use Follower instead of Slave and Leader instead of Master.

The Event Store .NET TCP Client 5.x has been updated in 5.0.8 to understand the new cluster terminology.
...
The intention here is to remove the need for having to use projections when you only want to subscribe to Event Store and only receive events from streams or events satisfying a particular filter.
...
Previously Event Store supported adding additional nodes to the cluster as clones (which as of Event Store 20.6.0 has been deprecated) for scaling out reads. This has led to a few issues as clones can be promoted into the cluster which can mean that clients end up reading from a node that starts participating in normal quorum operations. This has two undesired side effects, in that additional load can be inadvertently placed on a quorum node, and in the event of network segregation the cluster can end up in a split brain situation.
...
Both the Event Store TCP and gRPC Clients have also been adjusted to allow the user to set the node preference to ReadOnlyReplica.
...
We have introduced a health check which can be queried via the `{server_address}/health/live` endpoint. This endpoint will return a `204` status code once Event Store is ready to serve requests. 
...
To make configuration easier Event Store now supports multiple options for configuring certificates to allow you to choose what best suits your setup. The options provided have also been updated to have more detailed descriptions and errors to help guide customers during setup.
...
In the past Event Store has only supported using IP addresses in the gossip and advertise endpoints. We have now provided the ability to use hostname and port in the gossip seeds as well as the advertise endpoints.
