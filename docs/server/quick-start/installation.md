...
```bash
apt-get install kurrentdb=26.0.1
```
...
```bash
yum install kurrentdb-26.0.1-1.x86_64
```
...
```powershell
choco install kurrentdb --version=26.0.1
```
...
sc.exe create "KurrentDB" `
  start= delayed-auto     `
  binpath= "C:\ProgramData\chocolatey\lib\kurrentdb\kurrentdb-26.0.1-windows.x64\KurrentDB.exe --config=c:\path\to\kurrentdb-config.yaml"
```
