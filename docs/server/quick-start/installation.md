...
```bash
apt-get install eventstoredb-ee=24.10.12
```
...
```bash
yum install eventstoredb-ee-24.10.12-1.x86_64
```
...
```powershell
choco install eventstoredb-ee -s eventstore-eventstore --version 24.10.12
```
...
sc.exe create "EventStoreDB" `
  start= delayed-auto        `
  binpath= "C:\ProgramData\chocolatey\lib\eventstoredb-ee\eventstoredb-24.10.12-ee-windows.x64\EventStore.ClusterNode.exe --config=c:\path\to\eventstore-config.yaml"
```
