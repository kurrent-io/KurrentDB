...
The engine matters here. This tutorial uses V1, the engine KurrentDB runs by default, so `--engine-version 1` is explicit because gaffer would otherwise pick V2. V2 is an experimental alternative that processes partitions in parallel but does not emit result streams - see [Projections engine V2](./engine-v2.md).
...
- [Projections engine V2](./engine-v2.md) - the experimental parallel-partition engine, and how it differs from the engine used here.
