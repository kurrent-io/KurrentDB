...
However, there are some breaking changes since 22.10.0 that will affect external clients trying to connect to EventStoreDB 22.10.0 nodes:
...
This release includes packages for Ubuntu 22.04. While we have done preliminary testing on these packages, we have not yet run a full QA on them.
