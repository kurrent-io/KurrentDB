...
| Supported distance functions | `CosineDistance`, `CosineSimilarity`, `DotProductSimilarity`, `EuclideanSquaredDistance` (Lance's `cosine`/`dot`/`l2` — §8.6; `l2` is *squared* Euclidean, validated in-extension, hence `EuclideanSquaredDistance` not plain `EuclideanDistance`) |
...
**(Corrected 2026-07-17 — the original draft mirrored the pre-MEVD convention with `IKernelBuilder`/`serviceId`; current MEVD-era connectors use keyed services on `IServiceCollection` only.)** Current convention (verified against SK main's SqliteVec/PgVector): `Add{Name}VectorStore` / `AddKeyed{Name}VectorStore` in namespace `Microsoft.Extensions.DependencyInjection`, a `ServiceLifetime` parameter (default Singleton), no `IKernelBuilder` extensions, no `serviceId` (keyed DI services replace it). Store registration also registers the `VectorStore` base service descriptor. No per-collection DI extension in *this* connector (collections are `internal`-constructed via `GetCollection` — §5); note SK main's connectors *do* have `Add{Name}Collection` — that's a track-2 (upstream) delta, not a v1 concern.

| Method | Purpose |
|---|---|
| `DuckLanceServiceCollectionExtensions.AddDuckLanceVectorStore(IServiceCollection, DuckDBVectorStoreOptions options, ServiceLifetime lifetime = Singleton)` | Self-managed: constructs its own `Kurrent.Quack`-backed connection pool from `options.StoragePath`. |
| `DuckLanceServiceCollectionExtensions.AddKeyedDuckLanceVectorStore(IServiceCollection, object serviceKey, DuckDBVectorStoreOptions options, ServiceLifetime lifetime = Singleton)` | Keyed variant of the above. |
| Provider-delegate overloads of both | DI-resolved: expects a `Kurrent.Quack` pool/provider already registered in DI. Exact parameter shape depends on `Kurrent.Quack`'s own public types — finalized at Stage 12. |
...
| `DuckDBDynamicMapper` | Maps dynamic records (`Dictionary<string, object?>` via `GetDynamicCollection` — MEVD 10.x replaced the old `VectorStoreGenericDataModel<TKey>` with dynamic collections; corrected 2026-07-17). |
...
| `DuckDBFilterResult` | Holds the translated, *parameterized* WHERE predicate + its parameter values, plus whether it's prefilter-eligible (equality: yes, validated; `.Contains()`: no — the extension's filter IR can't push containment, so the executor switches to the oversample path — §8.5, Decision Log #21). |
...
| `DuckDBScoreConverter` | Converts `_distance` per the declared `DistanceFunction`'s own semantics (current MEVD convention — §8.6, Decision Log #1): `CosineDistance`/`EuclideanSquaredDistance` → raw `_distance`; `CosineSimilarity`/`DotProductSimilarity` → `1 - _distance`. |
...
| `Kurrent.Quack` | latest (pinned once resolved) | **Connection pool only — no other Quack facilities** (explicit decision 2026-07-17; e.g. `Kurrent.Quack.Arrow` is out of scope). Consumed as a NuGet package. Resolves the connection-thread-safety question that would otherwise be open (§6, `DuckDBConnectionManager`). Note the wider rule: this connector has **zero references to any `KurrentDB.*`/`Kurrent.Kontext.*` project** — packages only. |
...
- Ship two mappers: one for user POCOs, one for dynamic records (`Dictionary<string, object?>`, served via `GetDynamicCollection` — MEVD 10.x's replacement for the old `VectorStoreGenericDataModel<TKey>`; corrected 2026-07-17).
...
**Mechanism (validated live 2026-07-17 — corrects this spec's earlier draft):** the released extension has **no `filter :=` named parameter** on the search functions (on `main` it exists for `lance_vector_search`/`lance_fts` but is documented as REST-namespace-only; `lance_hybrid_search` has no filter parameter at all, even on `main`). Filters are expressed as a **plain, parameterized SQL `WHERE` clause** around the search table function, with `prefilter := true` — the extension pushes the predicate down into Lance. No filter-string building, no literal inlining, no injection surface: `DuckDBFilterTranslator` emits an ordinary parameterized predicate and `DuckDBFilterResult` carries SQL + parameter values.

| Shape | Example | SQL emitted | Prefilter status (validated) |
|---|---|---|---|
| Property equality | `r => r.Category == "x"` | `... FROM lance_vector_search(..., k := top+skip, prefilter := true) WHERE category = ?` | **True prefilter semantics**: full k matching rows, including matches far outside the global top-k; parameter-bound predicates push down fine; works through `lance_hybrid_search` too |
| List/tag containment | `r => r.Tags.Contains("x")` | `... FROM lance_vector_search(..., k := <oversampled>) WHERE array_has_any(tags, [?]) LIMIT <top>` | **Not prefilterable through the extension today**: its WHERE→Lance translator (`src/lance_filter_ir.cpp`) has no `array_has_any`/`list_contains` translation, so containment silently post-filters even with `prefilter := true` + a `LABEL_LIST` index (validated: 2 of 5 expected rows at k=5; no error raised; `use_index := false` doesn't change it). Connector strategy: **oversample** — large k, DuckDB evaluates the containment predicate itself, `LIMIT top` (validated correct). Same recall-bound caveat as §11's client-side-sort workaround. |

Lance's *own* filter layer does push `array_has_any`/`array_has_all` down with a `LABEL_LIST` index (the old `array_contains` gap, [lance#1115](https://github.com/lance-format/lance/issues/1115), was closed via #1793) — that's what LanceDB SDK filter strings use, and what the `filter :=` param will reach if it ships for local datasets; revisit the containment path then. Two clauses combine with `&&` (a containment clause forces the oversample path for the whole query). Only `IsIndexed = true` properties are filterable — reject or ignore filters on non-indexed properties, consistent with other connectors (per requirement 4.2 of the build guide).

**Note:** LanceDB's own client defaults pre-filtering to *on*; the DuckDB extension's search table functions default `prefilter` to `false`, and the difference is behaviorally large (validated: the same k=5 filtered query returns 5 rows with `prefilter := true`, fewer with `false`). Set it explicitly at the call site, always.
...
- `SearchAsync<TVector>` (via `lance_vector_search`): auto-select the vector property if only one exists; require `VectorPropertyName`/`VectorProperty` if there are multiple; throw if none exist. The query vector binds as a **prepared-statement parameter** — `CAST(? AS FLOAT[dim])` with a bound float list, validated identical to the literal form; no literal inlining needed.
- **Score conversion (re-resolved against current MEVD semantics — supersedes this spec's earlier `1/(1+d)` design):** current MEVD abandoned the blanket "higher = more similar" contract; `VectorSearchResult<TRecord>.Score` follows the semantics of the property's *declared* `DistanceFunction` (in-repo precedent: Redis/PgVector/SqliteVec return raw distance for distance functions and convert only for similarity functions). Lance's confirmed formulas (`cosine`/`dot` → `1 - x`, `l2` → *squared* Euclidean — validated in the extension itself: 0/2/4 no-index l2, 0/1/2 cosine) map as: `DistanceFunction.CosineDistance` → `_distance` as-is (it literally is cosine distance); `CosineSimilarity` → `1 - _distance`; `DotProductSimilarity` → `1 - _distance`; `EuclideanSquaredDistance` → `_distance` as-is (Lance `l2` *is* squared Euclidean — a perfect native fit); plain `EuclideanDistance` → `NotSupportedException` (or `sqrt(_distance)` if ever needed). The old `1/(1+d)` transform is dropped.
- **Quantized indexes require `refine_factor` (validated).** With `IVF_PQ`/`IVF_HNSW_PQ` and small k, quantization error scrambles ranking — a k=1 search for a vector *identical* to a stored row returned the wrong row until `refine_factor := 4` was passed; with it, distances are exact (0/1/2 for identical/orthogonal/opposite under cosine). `DuckDBVectorSearchExecutor` must always set `refine_factor` (default 4, tunable) when the collection's index kind is PQ-family.
...
| Mapping failure | MEVD's mapping-failure exception — **verify the exact 10.x type name against the pinned `Microsoft.Extensions.VectorData.Abstractions` package at Stage 1** (`VectorStoreRecordMappingException` was the earlier-preview name; unverified against 10.7.0) |
...
- `ALTER INDEX ... OPTIMIZE WITH (mode = 'append' | 'merge' | 'retrain')` — index freshness. **Syntax (validated):** `=` inside `WITH`, never `:=` (`:=` there produces `unknown field 'mode :'` — the options are JSON-encoded verbatim); `:=` is only for table-function arguments.
...
**Dataset-cache staleness caveat (validated 2026-07-17 + source-confirmed — constrains the scheduler's VACUUM policy):** the extension caches opened dataset handles per client context (`src/lance_dataset_cache.cpp`, keyed by table *and* by path). Writes/deletes/index-DDL invalidate cache entries, but **maintenance ops (`OPTIMIZE`, `VACUUM LANCE`, `ALTER INDEX ... OPTIMIZE`) never do** (`lance_maintenance.cpp` has no invalidation call). Reproduced live: an aggressive `VACUUM LANCE` (`older_than_seconds = 0, retain_n_versions = 1`) deleted data files that open sessions' cached search handles still referenced — every subsequent `lance_*_search` in those sessions (raw path or attached name) failed with `LanceError(IO): Not found: .../data/<file>.lance`. Plain retry, `DETACH`+`ATTACH`, and a row `DELETE` (table-key invalidation only) all stayed stale; in-session recovery came **only from index DDL** (invalidates the path key too) or a fresh connection; plain table scans (`SELECT`/`UPDATE`) were unaffected throughout. Connector policy: (1) the scheduler's `VACUUM LANCE` retention must stay comfortably longer than any plausible connection lifetime — with a sane multi-day window the failure mode is unreachable, since stale handles just read still-existing old-version files; (2) `DuckDBConnectionManager` treats `LanceError(IO) ... Not found` on a search as retryable-after-connection-recycle, as defense in depth.
...
- Embedding-generation-driven upsert/search against a real, SK-provided `IEmbeddingGenerator` (NuGet — e.g. Semantic Kernel's ONNX Bert generator with a small local model; the connector itself never implements or fakes generation, it only honors the abstraction's contract).
...
6. **Mapper layer** (§8.1) — POCO mapper, then the dynamic (`Dictionary<string, object?>`) mapper and `GetDynamicCollection` support.
...
**Status update (2026-07-17): the live-session validation this section called for has been executed.** A 56-check spike ran on osx_arm64 against DuckDB.NET.Data.Full **1.5.3** (DuckDB v1.5.3, `INSTALL lance` → extension build `533e0ee`), covering: extension bootstrap, `ATTACH`, `CREATE TABLE` (`VARCHAR[]` LIST + `FLOAT[4]` ARRAY columns), literal/parameterized/bulk `INSERT`, `MERGE INTO` upsert (both branches, parameter-bound), `UPDATE`/`DELETE`/`DROP TABLE`, vector/FTS/hybrid search, parameter-bound query vectors, `IVF_FLAT`/`IVF_PQ`/`IVF_HNSW_PQ`/`INVERTED`/`LABEL_LIST`/`BTREE` index DDL + `DROP INDEX`/`SHOW INDEXES`, distance-formula verification in-extension, prefilter semantics, scheduler primitives (`rows_indexed` staleness arithmetic, `append`/`retrain`, compaction, vacuum), and a direct-to-Lance storage proof (fresh in-memory connection re-attach sees everything; store dir contains only Lance artifacts, zero DuckDB files). Corrections found are folded into §8.4/§8.5/§8.6/§8.9 inline ("validated 2026-07-17" markers) — the substantive ones: `WITH (...)` takes `=` not `:=`; no `filter :=` param in the released build (filters = parameterized `WHERE` + `prefilter := true`; equality truly prefilters, containment does not — oversample fallback); quantized indexes require `refine_factor`; `SHOW INDEXES` has no timestamp field; maintenance ops don't invalidate the per-session dataset cache (VACUUM retention policy consequence).

**Resolved by the live run:**
- `ATTACH` on a not-yet-existing `StoragePath`: succeeds (doesn't create the directory; first `CREATE TABLE` does). A brand-new store's first collection needs no pre-created directory.
- Dual addressing: simpler than designed — the search functions accept the attached table name (`'ns.main.vs_docs'`) as their URI argument, so only index DDL strictly needs the raw dataset path. (Caveat: both URI forms share the same per-session dataset cache — see §8.9's staleness caveat.)
- Score conversion formulas: confirmed in the extension itself (no-index l2 → 0/2/4 squared; cosine → 0/1/2), not just via `pylance`.
- Compaction separate from index freshness: `OPTIMIZE` and `ALTER INDEX ... OPTIMIZE` both validated as separate, working statements.
- `updated_at_timestamp_ms`: does not exist in `SHOW INDEXES` output — stored last-retrain timestamp confirmed necessary (§8.9).

**Still open (unchanged by the live run):**
...
- Scalar data property type coverage (feature table, §1) — bool/int-widths/decimal/timestamp columns not yet exhaustively validated per-type (VARCHAR, VARCHAR[], FLOAT[N] validated).
- Multi-vector-per-record through the DuckDB extension specifically — full working test still `pylance`-only (structural rules confirmed from `index.rs`/`search.rs`).
...
- `IVF_PQ` `num_sub_vectors` divisibility validation — still `pylance`-confirmed only (the live run used valid combinations).
...
1. **Score sign convention (revised 2026-07-17).** Superseded: the original `1 - _distance` / `1/(1+d)` design targeted MEVD's old "higher = more similar" contract, which current MEVD abandoned — `Score` now follows the declared `DistanceFunction`'s own semantics (Redis/PgVector/SqliteVec precedent). Final mapping: `CosineDistance`/`EuclideanSquaredDistance` → `_distance` raw; `CosineSimilarity`/`DotProductSimilarity` → `1 - _distance`; plain `EuclideanDistance` → unsupported. Lance's `l2` being *squared* Euclidean makes `EuclideanSquaredDistance` a perfect native fit. Full detail: §8.6, `DuckDBScoreConverter` (§6).
...
12. **DI registration surface (revised 2026-07-17).** Superseded: the original four-method `IServiceCollection`/`IKernelBuilder` × `serviceId` design mirrored the pre-MEVD convention. Current MEVD convention (verified on SK main): `Add.../AddKeyed...` on `IServiceCollection` only, keyed services instead of `serviceId`, `ServiceLifetime` parameter, no `IKernelBuilder`. §6, "Dependency injection."
...
20. **Live validation executed (2026-07-17).** 56-check spike against DuckDB.NET.Data.Full 1.5.3 / DuckDB v1.5.3 / lance `533e0ee` on osx_arm64 — everything §15 asked for, plus the corrections below. Direct-to-Lance DML confirmed end-to-end (fresh-connection re-attach proof; store dir contains only Lance artifacts). §15.

21. **Filter mechanism (corrects the original §8.5).** No `filter :=` param in the released build; filters are parameterized `WHERE` clauses with `prefilter := true`. Equality predicates achieve true prefilter semantics (bound params included, hybrid included); list-containment (`array_has_any`) is not translated by the extension's filter IR (`lance_filter_ir.cpp`) and silently post-filters — connector uses the validated oversample fallback for tag filters. Eliminates the filter-string/injection design entirely. §8.5.

22. **Quantized search requires `refine_factor`.** Validated: `IVF_HNSW_PQ` k=1 returned the wrong row for an exact-match query until `refine_factor := 4`; with it, distances are exact. `DuckDBVectorSearchExecutor` always sets it for PQ-family indexes. §8.6.

23. **Maintenance doesn't invalidate the extension's dataset cache.** `OPTIMIZE`/`VACUUM`/`ALTER INDEX OPTIMIZE` never invalidate cached dataset handles (source: `lance_maintenance.cpp` vs `lance_index.cpp`/`lance_write.cpp`/`lance_delete.cpp`); an aggressive vacuum broke subsequent searches in open sessions (`LanceError(IO): Not found`), recoverable in-session only via index DDL or a fresh connection. Scheduler policy: conservative VACUUM retention (primary), plus recycle-connection-on-IO-Not-found as defense in depth. §8.9.
